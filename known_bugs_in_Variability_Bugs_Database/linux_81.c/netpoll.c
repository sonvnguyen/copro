...
typedef struct kfree { 
} kfree;
typedef struct __sum16 { 
} __sum16;
...
typedef struct __netpoll_cleanup { 
} __netpoll_cleanup;
typedef struct net_device { 
} net_device;
typedef struct inet6_ifaddr { 
} inet6_ifaddr;
...
typedef struct put_cpu_var { 
} put_cpu_var;
typedef struct softnet_data { 
} softnet_data;
typedef struct __wsum { 
} __wsum;
typedef struct net_device_ops { 
} net_device_ops;
typedef struct nd_msg { 
} nd_msg;
...
typedef struct netpoll_info { 
} netpoll_info;
typedef struct EXPORT_SYMBOL { 
} EXPORT_SYMBOL;
typedef struct EXPORT_SYMBOL_GPL { 
} EXPORT_SYMBOL_GPL;
typedef struct netpoll_info { 
} netpoll_info;
typedef struct sk_buff { 
...
typedef struct inet6_dev { 
} inet6_dev;
typedef struct netdev_queue { 
} netdev_queue;
typedef struct udelay { 
} udelay;
typedef struct napi_struct { 
} napi_struct;
typedef struct gfp_t { 
} gfp_t;
...
#define MAX_SKB_SIZE							 	(sizeof(struct ethhdr) +					 	 sizeof(struct iphdr) +						 	 sizeof(struct udphdr) +					 	 MAX_UDP_CHUNK)
...
static void queue_process(struct work_struct *work)
{
	struct netpoll_info *npinfo =
		container_of(work,  tx_work.work);
	struct sk_buff *skb;
	unsigned long flags;
...
{
	struct netpoll *np = container_of(rcu_head, rcu);
